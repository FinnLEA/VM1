...
my $op = shift @_;
	...
	elsif ($op =~ /^(byte )* *\[0x([a-fA-F0-9]+)\]$/) {
        if (defined ($1)) {
            return $optypes{"memb"}.pack("h8",scalar reverse($2));
            }
        else {
            return $optypes{"memw"}.pack("h8",scalar reverse($2));
            }
        }
	elsif ($op =~ /^(byte )* *\[r([0-9]+)\]$/) {
        if (defined ($1)) {
            return $optypes{"memrb"}.pack("L",$2);
            }
        else {
            return $optypes{"memrw"}.pack("L",$2);
            }
        }
